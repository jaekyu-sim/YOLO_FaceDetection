...
class Optimizer(metaclass=ABCMeta):
	...
	def _step(self, sess, **kwargs):
		...
		_, loss, y_pred = 			sess.run([self.optimize, self.model.loss, self.model.pred_y],
			...
			if (i) % num_steps_per_epoch == 0:
				...
				if self.val_set is not None:
					...
					if verbose:
						...
						print('[epoch {}]\tloss: {:.6f} |Train score: {:.6f} |Eval score: {:.6f} |lr: {:.6f}'							.format(self.curr_epoch, step_loss, step_score, eval_score, self.curr_learning_rate))
					...
					if verbose:
						# Print intermediate results
						print('[epoch {}]\tloss: {:.6f} |Train score: {:.6f} |lr: {:.6f}'							.format(self.curr_epoch, step_loss, step_score, self.curr_learning_rate))
						# Plot intermediate results
